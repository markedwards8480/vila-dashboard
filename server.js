...
app.post('/api/chat', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const statements = await pool.query(`
      SELECT id, year, month, closing_balance, owner_nights, guest_nights, 
             rental_nights, vacant_nights, rental_revenue, owner_revenue_share,
             statement_date
      FROM monthly_statements 
      ORDER BY year DESC, month DESC
    `);
    
    // 2. All expenses with month/year context
    const expenses = await pool.query(`
      SELECT ec.id, ec.category, ec.subcategory, ec.amount, 
             ms.year, ms.month
    ...
    // 3. Expense summaries by category (all time)
    const expenseSummary = await pool.query(`
      SELECT ec.category, ec.subcategory, 
    ...
    const monthlyTotals = await pool.query(`
      SELECT ms.year, ms.month, 
             ...
             ms.closing_balance,
             ms.owner_revenue_share
      FROM monthly_statements ms
      LEFT JOIN expense_categories ec ON ms.id = ec.statement_id
      GROUP BY ms.id, ms.year, ms.month, ms.closing_balance, ms.owner_revenue_share
      ORDER BY ms.year DESC, ms.month DESC
    `);
    
    // 5. All utility readings
    const utilities = await pool.query(`
      SELECT ur.utility_type, ur.consumption, ur.cost, ur.unit,
             ms.year, ms.month
      FROM utility_readings ur
      JOIN monthly_statements ms ON ur.statement_id = ms.id
      ORDER BY ms.year DESC, ms.month DESC
    `);
    
    // 6. Occupancy details
    const occupancy = await pool.query(`
      SELECT od.activity_type, od.check_in, od.check_out, od.num_nights,
             ms.year, ms.month
      FROM occupancy_details od
      JOIN monthly_statements ms ON od.statement_id = ms.id
      ORDER BY od.check_in DESC
    `);
    
    // 7. Occupancy summary by year
    const occupancySummary = await pool.query(`
      SELECT year,
             SUM(owner_nights) as total_owner_nights,
             SUM(guest_nights) as total_guest_nights,
             SUM(rental_nights) as total_rental_nights,
             SUM(vacant_nights) as total_vacant_nights,
             SUM(owner_revenue_share) as total_revenue
      FROM monthly_statements
      GROUP BY year
      ORDER BY year DESC
    `);
    ...
    const folios = await pool.query(`
      SELECT id, folio_date, guest_name, total_charges
      FROM hotel_folios
      ORDER BY folio_date DESC
      LIMIT 50
    `);
    
    // 9. Folio line items
    const folioItems = await pool.query(`
      SELECT fli.description, fli.category, fli.amount, hf.folio_date, hf.guest_name
      FROM folio_line_items fli
      JOIN hotel_folios hf ON fli.folio_id = hf.id
      ORDER BY hf.folio_date DESC
      LIMIT 200
    `);
    ...
    const systemPrompt = `You are a knowledgeable financial assistant for Villa 8 at Amanyara Resort in Turks & Caicos. 
You have COMPLETE access to all financial data in the database. Use this data to answer questions accurately and specifically.

=== MONTHLY STATEMENTS (${statements.rows.length} total) ===
${statements.rows.map(s => 
  `${monthNames[s.month-1]} ${s.year}: Balance=$${s.closing_balance || 0}, Owner Nights=${s.owner_nights || 0}, Guest=${s.guest_nights || 0}, Rental=${s.rental_nights || 0}, Vacant=${s.vacant_nights || 0}, Revenue Share=$${s.owner_revenue_share || 0}`
).join('\n')}

=== EXPENSE SUMMARY BY CATEGORY (All Time) ===
${expenseSummary.rows.map(e => 
  `${e.category} - ${e.subcategory}: Total=$${parseFloat(e.total_amount).toFixed(2)}, Avg=$${parseFloat(e.avg_amount).toFixed(2)}, Count=${e.occurrence_count}`
).join('\n')}

=== MONTHLY EXPENSE TOTALS ===
${monthlyTotals.rows.map(m => 
  `${monthNames[m.month-1]} ${m.year}: Expenses=$${parseFloat(m.total_expenses || 0).toFixed(2)}, Balance=$${m.closing_balance || 0}, Revenue=$${m.owner_revenue_share || 0}`
).join('\n')}

=== ALL EXPENSE LINE ITEMS (${expenses.rows.length} total) ===
${expenses.rows.map(e => 
  `${monthNames[e.month-1]} ${e.year}: ${e.category}/${e.subcategory} = $${parseFloat(e.amount).toFixed(2)}`
).join('\n')}

=== UTILITY COSTS ===
${utilities.rows.map(u => 
  `${monthNames[u.month-1]} ${u.year}: ${u.utility_type} = $${parseFloat(u.cost || 0).toFixed(2)}`
).join('\n')}

=== OCCUPANCY SUMMARY BY YEAR ===
${occupancySummary.rows.map(o => 
  `${o.year}: Owner=${o.total_owner_nights || 0} nights, Guest=${o.total_guest_nights || 0}, Rental=${o.total_rental_nights || 0}, Vacant=${o.total_vacant_nights || 0}, Revenue=$${o.total_revenue || 0}`
).join('\n')}

=== DETAILED OCCUPANCY (Check-in/Check-out) ===
${occupancy.rows.slice(0, 50).map(o => 
  `${o.activity_type}: ${o.check_in ? new Date(o.check_in).toLocaleDateString() : 'N/A'} to ${o.check_out ? new Date(o.check_out).toLocaleDateString() : 'N/A'} (${o.num_nights} nights)`
).join('\n')}

${folios.rows.length > 0 ? `
=== HOTEL FOLIOS ===
${folios.rows.map(f => 
  `${f.folio_date ? new Date(f.folio_date).toLocaleDateString() : 'N/A'}: ${f.guest_name || 'Unknown'} - $${f.total_charges || 0}`
).join('\n')}
` : ''}

${folioItems.rows.length > 0 ? `
=== FOLIO LINE ITEMS ===
${folioItems.rows.map(f => 
  `${f.folio_date ? new Date(f.folio_date).toLocaleDateString() : 'N/A'} (${f.guest_name || 'Unknown'}): ${f.category || 'Other'} - ${f.description} = $${f.amount || 0}`
).join('\n')}
` : ''}

INSTRUCTIONS:
- Always provide specific numbers from the data
- Calculate totals, averages, and comparisons when asked
- Reference specific months and years
- If data is missing or unavailable, say so
- Be concise but thorough
- Format currency with $ and 2 decimal places
- When comparing periods, show the actual values`;
